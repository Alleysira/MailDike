...
//        }
    ...
    private void runPerformanceTest() {
        ...
        out.println("Test Self-Extractable BE engine: " + engine.getEngineName());
    ...
    private void run_one_round() {
        try {
            ...
            for (int i = 0; i < maximal_users; i++) {
                ...
                timer.start(i);
                headers[i] = engine.encapsulation(publicKey, indexSets[i], eks[i]).getHeader();
                temperTime = timer.stop(i);
                out.print("\t" + temperTime);
...
//        performanceTest.test_round = TestUtils.DEFAULT_SIMU_TEST_ROUND;
        ...
        performanceTest.engine = new SelfExtractableBEEngine(beEngine, pbeParametersGenerator, blockCipher, digest);
