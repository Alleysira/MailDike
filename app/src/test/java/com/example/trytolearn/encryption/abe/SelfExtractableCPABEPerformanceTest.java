...
public class SelfExtractableCPABEPerformanceTest extends TestCase {
    ...
    //self secret key generation time
...
//        }
    ...
    private void runPerformanceTest() {
        ...
        out.println("Test Self-Extractable CP-ABE engine: " + engine.getEngineName());
    ...
    private void run_one_round() {
        try {
            ...
            for (int i = 0; i < maximal_attributes; i++) {
                ...
                timer.start(i);
                headers[i] = engine.encapsulation(publicKey, accessPolicies[i], eks[i]).getHeader();
                temperTime = timer.stop(i);
                out.print("\t" + temperTime);
...
//        performanceTest.test_round = TestUtils.DEFAULT_SIMU_TEST_ROUND;
        ...
        performanceTest.engine = new SelfExtractableCPABEEngine(cpabeEngine, pbeParametersGenerator, blockCipher, digest);
